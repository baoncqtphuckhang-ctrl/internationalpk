...
import { FileSignature, CheckCircle2, Clock, XCircle, DollarSign, User, FileText, Send, Check, X, Trash2, Tag, Archive, AlertCircle } from 'lucide-react';
...
export default function ApprovalWorkflow({ 
    ...
    return (
        <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
            ...
            {view === 'create' ? (
                <div className="bg-white p-8 md:p-12 shadow-lg rounded-xl font-['Times_New_Roman',_serif] text-[15px] text-black border border-slate-200 w-full max-w-3xl mx-auto">
                    <div className="text-center mb-8"><h1 className="font-bold text-[17px] uppercase tracking-wide">CÔNG TY TNHH TM XD TTNT QUỐC TẾ PHÚC KHANG</h1><p>72/5 Trần Đình Xu, Phường Cô Giang, Q1, TP HCM</p></div>
                    
                    <div className="mb-4 flex gap-6 bg-white p-3 rounded-xl border border-slate-200 w-max shadow-sm mx-auto">
                        <label className="flex items-center gap-2 cursor-pointer font-bold text-slate-700 font-sans">
                    ...
                    <div className="text-center mb-6"><h2 className="text-2xl font-bold mb-2 uppercase">{dnttData.docType === 'DNTT' ? 'ĐỀ NGHỊ THANH TOÁN' : 'ĐỀ NGHỊ TẠM ỨNG'}</h2>
                    ...
                    <table className={`w-full border-collapse border border-black mb-12 ${dnttData.paymentMethod === 'tien_mat' ? 'opacity-30' : ''}`}>
                    ...
                    <div className="flex gap-2 font-sans border-t pt-6">
                        <button onClick={() => setView('list')} className="px-6 py-2.5 rounded-xl font-bold transition bg-slate-100 text-slate-600 hover:bg-slate-200 flex-1">
                            Hủy bỏ
                        </button>
                        <button onClick={handleSubmit} disabled={isLoading} className="bg-blue-600 text-white font-bold py-2.5 px-6 rounded-xl shadow-lg hover:bg-blue-700 transition flex-1 flex items-center justify-center gap-2">
                ...
                <div className="space-y-6">
                    <div className="flex border-b border-slate-200 bg-slate-100 p-1 rounded-2xl w-fit">
                        <button 
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {dnttList.filter(d => {
                            if (filter === 'active') return ![STATUSES.PAID, STATUSES.ACCOUNTED, STATUSES.REJECTED].includes(d.status);
                            if (filter === 'paid') return d.status === STATUSES.PAID;
                            return d.status === STATUSES.ACCOUNTED;
                        }).length === 0 ? (
                            ...
                            dnttList.filter(d => {
                                ...
                                return (
                                    <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:shadow-md transition-all duration-300">
                                        ...
                                        <div className="flex gap-3 w-full md:w-auto">
                                            {activeTab === 'approvals' && item.status === STATUSES.WAITING_QS && canApproveQS && (
                                                <>
                                                    <button onClick={() => onUpdateStatus(item.id, item.doc_type === 'Đơn Vật Tư' ? STATUSES.PAID : STATUSES.WAITING_ACC)} className="flex-1 md:flex-none bg-blue-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-blue-700 transition flex items-center gap-2 justify-center shadow-lg shadow-blue-600/20"><Check size={18}/> Duyệt (QS)</button>
                                                    <button onClick={() => onUpdateStatus(item.id, STATUSES.REJECTED)} className="flex-1 md:flex-none bg-red-50 text-red-600 px-6 py-2 rounded-xl font-bold hover:bg-red-600 hover:text-white transition flex items-center gap-2 justify-center border border-red-100"><X size={18}/> Từ chối</button>
                                                </>
                                            )}
                                            {activeTab === 'approvals' && item.status === STATUSES.WAITING_ACC && canApproveKT && (
                                                <>
                                                    <button onClick={() => onUpdateStatus(item.id, STATUSES.APPROVED)} className="flex-1 md:flex-none bg-green-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-green-700 transition flex items-center gap-2 justify-center shadow-lg shadow-green-600/20"><Check size={18}/> Duyệt (KT)</button>
                                                    <button onClick={() => onUpdateStatus(item.id, STATUSES.REJECTED)} className="flex-1 md:flex-none bg-red-50 text-red-600 px-6 py-2 rounded-xl font-bold hover:bg-red-600 hover:text-white transition flex items-center gap-2 justify-center border border-red-100"><X size={18}/> Từ chối</button>
                                                </>
                                            )}
                                            {activeTab === 'approvals' && item.status === STATUSES.APPROVED && canPay && (
                                                <button 
                                                    ...
                                                    className="flex-1 md:flex-none bg-indigo-600 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-indigo-700 transition flex items-center gap-2 justify-center shadow-lg shadow-indigo-600/20"
                                                >
                                                    <DollarSign size={18}/> Xác nhận Chi tiền
                                                </button>
                                            )}
                                            {activeTab === 'approvals' && item.status === STATUSES.PAID && canAccount && (
                                                <button 
                                                    onClick={() => openDistributeModal(item)} 
                                                    className="flex-1 md:flex-none bg-amber-500 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-amber-600 transition flex items-center gap-2 justify-center shadow-lg shadow-amber-500/20 animate-pulse hover:animate-none"
                                                >
            ...
            {distributeItem && (
                <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[1000] flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <header className="bg-slate-900 text-white p-8 flex justify-between items-start">
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    <Archive className="text-amber-500" />
                                    <h3 className="text-2xl font-black uppercase tracking-tight">Hạch Toán Chi Phí</h3>
                                </div>
                        ...
                        <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar bg-slate-50">
                            ...
                            <div className="space-y-4">
                                {distributionData.map((d, idx) => (
                                    <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-center">
                                        <div className="flex-1">
                                            <p className="text-xs text-slate-400 font-bold uppercase mb-1">Nội dung đề nghị {idx + 1}</p>
                                            <p className="font-bold text-slate-800 leading-tight">{d.content}</p>
                        ...
                        <footer className="p-8 bg-white border-t-2 border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div className="flex items-center gap-4 bg-amber-50 px-6 py-3 rounded-2xl border border-amber-100">
                                ...
                                <span className="text-2xl font-black text-amber-900">{formatCurrency(distributionData.reduce((sum, d) => sum + d.amount, 0))}</span>
                            </div>
                            <div className="flex gap-4 w-full md:w-auto">
                                <button 
                                    onClick={() => setDistributeItem(null)}
                                    className="flex-1 md:flex-none px-8 py-3.5 rounded-2xl font-bold text-slate-500 hover:bg-slate-100 transition"
                                >
                                    Để sau
                                </button>
                                <button 
                                    onClick={handleSaveDistribution}
                                    className="flex-1 md:flex-none px-12 py-3.5 rounded-2xl font-black bg-indigo-600 text-white shadow-xl shadow-indigo-600/30 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all duration-200"
                                >
                                    HOÀN TẤT HẠCH TOÁN
                                </button>
                            </div>
