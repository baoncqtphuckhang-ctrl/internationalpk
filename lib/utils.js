...
export const EXPENSE_CATEGORIES = [
    ...
    { code: '131', name: 'Công nợ phải thu' },
];
...
export const formatCurrency = (amount) => {
    ...
    let cleanStr = amount.toString();
    const num = Number(amount);
    if (!isNaN(num)) {
        cleanStr = Number(num.toFixed(4)).toString();
    }
...
export const parseVietnameseNumber = (str) => {
    ...
    let multiplier = 1;
    let stringToProcess = s;

    // Hỗ trợ nhập nhanh k (nghìn), m/tr (triệu), b/t (tỷ)
    if (s.endsWith('k')) {
        multiplier = 1000;
        stringToProcess = s.slice(0, -1);
    } else if (s.endsWith('m') || s.endsWith('tr')) {
        multiplier = 1000000;
        stringToProcess = s.replace(/m$|tr$/, '');
    } else if (s.endsWith('b') || s.endsWith('t')) {
        multiplier = 1000000000;
        stringToProcess = s.replace(/b$|t$/, '');
    }

    let cleaned = stringToProcess.replace(/[^0-9.,-]/g, '');
    if (!cleaned) return '';
    
    const commaCount = (cleaned.match(/,/g) || []).length;
    const dotCount = (cleaned.match(/\./g) || []).length;

    // Nếu gõ 800,000 (1 dấu phẩy, theo sau là 3 chữ số) -> hiểu là phân cách hàng nghìn
    if (commaCount === 1 && dotCount === 0 && cleaned.match(/,\d{3}$/)) {
    ...
    return isNaN(val) ? '' : val * multiplier;
};
...
// Dynamic storage public URL resolver (hỗ trợ đường dẫn tương đối và tự động tương thích khi đổi domain Supabase)
export const getStoragePublicUrl = (pathOrUrl = '') => {
    if (!pathOrUrl) return '';
    ...
    const match = pathOrUrl.match(/\/storage\/v1\/object\/public\/(.+)$/i) || pathOrUrl.match(/\/object\/public\/(.+)$/i);
    if (match && match[1]) {
        storagePath = match[1];
    ...
    if (!activeUrl) return pathOrUrl;

    if (cleanPath.startsWith('invoices/')) {
        return `${activeUrl}/storage/v1/object/public/${cleanPath}`;
    }
    return `${activeUrl}/storage/v1/object/public/invoices/${cleanPath}`;
};

export const normalizePdfUrl = getStoragePublicUrl;

// Hàm so sánh và sắp xếp thứ tự đợt thanh toán chuẩn (Tạm ứng -> Đợt 1..N -> Quyết toán -> GTBL)
export const comparePhases = (a = '', b = '') => {
    const normA = String(a || '').trim().toLowerCase();
    const normB = String(b || '').trim().toLowerCase();
    
    if (normA === normB) return 0;

    const isAdvanceA = normA.includes('tạm ứng') || normA === 'tam ung';
    const isAdvanceB = normB.includes('tạm ứng') || normB === 'tam ung';
    if (isAdvanceA && !isAdvanceB) return -1;
    if (!isAdvanceA && isAdvanceB) return 1;

    const isGtblA = normA.includes('gtbl') || normA.includes('bảo hành');
    const isGtblB = normB.includes('gtbl') || normB.includes('bảo hành');

    const isSettlementA = normA.includes('quyết toán') || normA.includes('quyet toan');
    const isSettlementB = normB.includes('quyết toán') || normB.includes('quyet toan');

    if (isGtblA && !isGtblB) return 1;
    if (!isGtblA && isGtblB) return -1;

    if (isSettlementA && !isSettlementB) return isGtblB ? -1 : 1;
    if (!isSettlementA && isSettlementB) return isGtblA ? 1 : -1;

    const numA = parseFloat(normA.match(/\d+(\.\d+)?/)?.[0] || '0');
    const numB = parseFloat(normB.match(/\d+(\.\d+)?/)?.[0] || '0');
...
export const sortPhases = (phases = []) => {
    return [...new Set((phases || []).filter(Boolean))].sort(comparePhases);
};
